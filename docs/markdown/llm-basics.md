---
title: LLM Basics
description: Essential concepts for working with language models in Quaynor
sidebar_title: LLM Basics
order: 2
---

Our goal with Quaynor is to make it easy to run local LLMs. For this reason we have made it possible to use Quaynor with minimal knowledge of how LLM works. However you still need to know some basic concepts, so for these we provide some brief explanations. The concepts covered are tokens, context, samplers and tools. 
...
When the model generates text, it produces one token at a time. This is why the default response object of Quaynor is a stream of tokens and why you can read the response token-by-token.
...
Quaynor also supports more advanced ways of configuraing a sampler, like for example follow a JSON Schema.
