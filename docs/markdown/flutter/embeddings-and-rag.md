...
description: Learn how to use embeddings and cross-encoders to build retrieval-augmented generation (RAG) systems with Quaynor.
...
final encoder = await quaynor.Encoder.fromPath(modelPath: './embedding-model.gguf');
...
final similarity1 = quaynor.cosineSimilarity(
  a: query.toList(),
  b: doc1.toList()
);
final similarity2 = quaynor.cosineSimilarity(
  a: query.toList(),
  b: doc2.toList()
);
...
for (int i = 0; i < docEmbeddings.length; i++) {
  final similarity = quaynor.cosineSimilarity(
