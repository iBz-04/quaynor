---
title: Tool Calling
description: An introduction to tools in Quaynor
...
!!! info ""
    ...
    For reliable tool calling, we recommend trying the [Qwen 3 family](https://huggingface.co/collections/Qwen/qwen3).
...
```typescript
import { Tool } from "react-native-quaynor";
...
Quaynor then figures out the right tool calling format and configures the sampler.

Naturally, more tools can be defined and the model can chain the calls for them:

```typescript
import { Chat, Tool } from "react-native-quaynor";
