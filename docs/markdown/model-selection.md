...
description: Quaynor is a lightweight, open-source AI engine for local LLM inference. Simple, privacy oriented with no infrastructure needed.
...
Pass these as the model path when creating a `Chat`, `Model`, `Encoder`, etc. Quaynor will download the model automatically and cache it locally for future use.


## Getting a model

Quaynor can download models directly from Hugging Face. Instead of downloading a file manually, pass a `huggingface:` path where you'd normally pass a file path:
...
Good GGUF mirrors to start from include [**Bartowski**](https://huggingface.co/bartowski), [**Unsloth**](https://huggingface.co/unsloth/models), and the [**Qwen**](https://huggingface.co/Qwen) model hub—the examples above come from Bartowski mirrors of Qwen releases.
...
For chatting, you'll need an instruction-tuned GGUF file that includes a Jinja2 chat template in its metadata. This describes the vast majority of GGUF files available, so if you're unsure, just try it — Quaynor will give you a descriptive error message if something isn't right.
