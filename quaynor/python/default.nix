...
let
  ...
  quaynor-python-rs = workspace.workspaceMembers.quaynor-python.build;
in
python3Packages.buildPythonPackage {
  ...
  installPhase = ''
    ...
    cp ${quaynor-python-rs.lib}/lib/libquaynor_python.so \
       $out/${python3.sitePackages}/quaynor.abi3.so
    runHook postInstall
  '';
  ...
  # Path(__file__).parent.parent / "quaynor" / "python" / "tests" / "img"
  preCheck = ''
    # The "mkdir -p quaynor/..." below creates a quaynor/ directory that Python 3
    ...
    cp -r ${../../docs} docs
    mkdir -p quaynor/python/tests
    ln -s ../../../tests/img quaynor/python/tests/img
