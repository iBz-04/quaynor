/// This file builds stubs for the quaynor-python crate.
/// It uses the pyo3-introspection crate to introspect the quaynor-python crate and
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let library_path = format!("../target/{}/libquaynor_python.{}", profile, lib_ext);
    ...
    let module = introspect_cdylib(&library_path, "quaynor")?;
    ...
    // Move __init__.pyi to quaynor.pyi
    let init_path = PathBuf::from(&output_dir).join("__init__.pyi");
    let target_path = PathBuf::from(&output_dir).join("quaynor.pyi");
