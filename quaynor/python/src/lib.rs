...
impl Model {
    ...
    pub fn new(
        ...
        let mmproj_str = projection_model_path
            ...
            .transpose()?;
        let model_result = quaynor::llm::get_model(path_str, use_gpu_if_available, mmproj_str);
        match model_result {
            Ok(model) => Ok(Self {
                model: Arc::new(model),
            }),
    ...
    pub async fn load_model_async(
        ...
        let mmproj_str = projection_model_path
            ...
            .transpose()?;
        let model_result = quaynor::llm::get_model_async(
            path_str.to_owned(),
...
/// to make the simplest possible usage of quaynor even simpler
...
impl<'py> ModelOrPath<'py> {
    /// returns quaynor core's internal model struct from a python `str | Model`
    fn get_inner_model(&self) -> PyResult<Arc<quaynor::llm::Model>> {
        match self {
            ...
            ModelOrPath::Path(path) => {
                ...
                quaynor::llm::get_model(path_str, true, None)
...
pub struct TokenStream {
    stream: quaynor::chat::TokenStream,
}
...
pub struct TokenStreamAsync {
    ...
    stream: std::sync::Arc<tokio::sync::Mutex<quaynor::chat::TokenStreamAsync>>,
...
#[pyclass]
pub struct Encoder {
    encoder: Option<quaynor::encoder::Encoder>,
}

impl Encoder {
    fn inner(&self) -> &quaynor::encoder::Encoder {
...
impl Encoder {
    ...
    pub fn new(model: ModelOrPath, n_ctx: u32) -> PyResult<Self> {
        ...
        let encoder = quaynor::encoder::Encoder::new(nw_model, n_ctx);
        Ok(Self {
...
pub struct EncoderAsync {
    encoder_handle: Option<quaynor::encoder::EncoderAsync>,
}

impl EncoderAsync {
    fn inner(&self) -> &quaynor::encoder::EncoderAsync {
        self.encoder_handle
            .as_ref()
...
impl EncoderAsync {
    ...
    pub fn new(model: ModelOrPath, n_ctx: u32) -> PyResult<Self> {
        let nw_model = model.get_inner_model()?;
        let encoder_handle = quaynor::encoder::EncoderAsync::new(nw_model, n_ctx);
        Ok(Self {
...
pub struct CrossEncoder {
    crossencoder: Option<quaynor::crossencoder::CrossEncoder>,
}

impl CrossEncoder {
    fn inner(&self) -> &quaynor::crossencoder::CrossEncoder {
        self.crossencoder
            .as_ref()
...
impl CrossEncoder {
    ...
    pub fn new(model: ModelOrPath, n_ctx: u32) -> PyResult<Self> {
        let nw_model = model.get_inner_model()?;
        let crossencoder = quaynor::crossencoder::CrossEncoder::new(nw_model, n_ctx);
        Ok(Self {
...
pub struct CrossEncoderAsync {
    crossencoder_handle: Option<quaynor::crossencoder::CrossEncoderAsync>,
}

impl CrossEncoderAsync {
    fn inner(&self) -> &quaynor::crossencoder::CrossEncoderAsync {
...
impl CrossEncoderAsync {
    ...
    pub fn new(model: ModelOrPath, n_ctx: u32) -> PyResult<Self> {
        let nw_model = model.get_inner_model()?;
        let crossencoder_handle = quaynor::crossencoder::CrossEncoderAsync::new(nw_model, n_ctx);
        Ok(Self {
...
pub struct Chat {
    ...
    chat_handle: Option<quaynor::chat::ChatHandle>,
}

impl Chat {
    fn handle(&self) -> &quaynor::chat::ChatHandle {
...
impl Chat {
    ...
    pub fn new(
        ...
        let chat_handle = quaynor::chat::ChatBuilder::new(nw_model)
...
pub struct ChatAsync {
    ...
    chat_handle: Option<quaynor::chat::ChatHandleAsync>,
}

impl ChatAsync {
    fn handle(&self) -> &quaynor::chat::ChatHandleAsync {
        self.chat_handle
            .as_ref()
...
fn cosine_similarity(a: Vec<f32>, b: Vec<f32>) -> PyResult<f32> {
    ...
    Ok(quaynor::encoder::cosine_similarity(&a, &b))
}
...
#[derive(Clone)]
pub struct Prompt {
    prompt: quaynor::tokenizer::Prompt,
}
...
fn python_tool(
    ...
    let core_tool = quaynor::tool_calling::Tool::python(
...
fn bash_tool(max_commands: Option<usize>, py: Python) -> PyResult<Tool> {
    let core_tool = quaynor::tool_calling::Tool::bash(max_commands);
...
fn python_func_json_schema(
    ...
    if let Some(missing_arg) = args.iter().find(|arg| !annotations.contains_key(*arg)) {
        return Err(pyo3::exceptions::PyTypeError::new_err(format!(
            "ERROR: Parameter '{missing_arg}' is missing a type hint. Quaynor requires all tool function parameters to have static type hints. E.g.: `{missing_arg}: str`"
        )));
    }
...
#[pymodule(name = "quaynor")]
pub mod quaynorpython {
    use pyo3::prelude::*;
