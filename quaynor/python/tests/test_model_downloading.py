...
# This translates to: https://huggingface.co/bartowski/Qwen_Qwen3-0.6B-GGUF/resolve/main/Qwen_Qwen3-0.6B-Q4_K_M.gguf
HF_MODEL = "hf://bartowski/Qwen_Qwen3-0.6B-GGUF/Qwen_Qwen3-0.6B-Q4_K_M.gguf"
...
HF_NONEXISTENT_MODEL = "hf://this-org-definitely-does-not-exist-00042/fake-repo/model.gguf"
...
def test_download_model():
    ...
    model = quaynor.Model(HF_MODEL)
    assert isinstance(model, quaynor.Model)
...
def test_download_model_chat():
    """A model can be loaded using an hf:// URL; this downloads it if needed."""
    model = quaynor.Chat(HF_MODEL)
    assert isinstance(model, quaynor.Chat)
...
def test_local_path_does_not_trigger_download():
    ...
    with pytest.raises(RuntimeError, match="Model not found"):
        quaynor.Model("/this/path/does/not/exist/model.gguf")
...
def test_chat_triggers_hf_download_path():
    with pytest.raises(RuntimeError) as exc_info:
        quaynor.Chat(HF_NONEXISTENT_MODEL)
...
def test_chat_async_triggers_hf_download_path():
    with pytest.raises(RuntimeError) as exc_info:
        quaynor.ChatAsync(HF_NONEXISTENT_MODEL)
    _assert_triggers_hf_download_path(exc_info.value)
...
def test_encoder_triggers_hf_download_path():
    with pytest.raises(RuntimeError) as exc_info:
        quaynor.Encoder(HF_NONEXISTENT_MODEL)
    _assert_triggers_hf_download_path(exc_info.value)
...
def test_encoder_async_triggers_hf_download_path():
    with pytest.raises(RuntimeError) as exc_info:
        quaynor.EncoderAsync(HF_NONEXISTENT_MODEL)
    _assert_triggers_hf_download_path(exc_info.value)
...
def test_crossencoder_triggers_hf_download_path():
    with pytest.raises(RuntimeError) as exc_info:
        quaynor.CrossEncoder(HF_NONEXISTENT_MODEL)
    _assert_triggers_hf_download_path(exc_info.value)
...
def test_crossencoder_async_triggers_hf_download_path():
    with pytest.raises(RuntimeError) as exc_info:
        quaynor.CrossEncoderAsync(HF_NONEXISTENT_MODEL)
    _assert_triggers_hf_download_path(exc_info.value)


@pytest.mark.network
def test_model_triggers_hf_download_path():
    """quaynor.Model() with hf:// also routes through the download path."""
    with pytest.raises(RuntimeError) as exc_info:
        quaynor.Model(HF_NONEXISTENT_MODEL)
    _assert_triggers_hf_download_path(exc_info.value)
