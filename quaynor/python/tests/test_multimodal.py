...
def multimodal_model():
    ...
    return quaynor.Model(model_path, projection_model_path=image_model_path)
...
def test_multiple_images(multimodal_chat):
    ...
    prompt = quaynor.Prompt(
        [
            quaynor.Image(image_paths[0]),
            quaynor.Image(image_paths[1]),
            quaynor.Text(
...
def test_multiple_images_interleaved(multimodal_chat):
    ...
    prompt = quaynor.Prompt(
        [
            quaynor.Text("What animal is in the first image?"),
            quaynor.Image(image_paths[0]),
            quaynor.Text("What animal is in the second image?"),
            quaynor.Image(image_paths[1]),
            quaynor.Text(
...
def test_audio_transcription(multimodal_chat):
    ...
    prompt = quaynor.Prompt([
        quaynor.Text("Please transcribe this audio."),
        quaynor.Audio(audio_path),
    ])
    response = multimodal_chat.ask(prompt).completed()
...
def test_audio_transcription_and_image_ingestion(multimodal_chat):
    ...
    image_path = os.path.join(os.path.dirname(__file__), "img/dog.png")
    prompt = quaynor.Prompt([
        quaynor.Text("Please transcribe this audio and describe the image."),
        quaynor.Audio(audio_path),
        quaynor.Image(image_path),
    ])
    response = multimodal_chat.ask(prompt).completed()
