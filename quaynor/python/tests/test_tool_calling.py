...
@quaynor.tool(description="Applies the sparklify effect to a given piece of text.")
...
@quaynor.tool(description="Returns the intersection of set1 and set2")
...
def test_tool_construction():
    ...
    assert isinstance(sparklify, quaynor.Tool)
...
def test_tool_bad_parameters():
    with pytest.raises(TypeError):
        ...
        @quaynor.tool(description="foobar", params={"b": "uh-oh"})
...
async def test_async_tool_construction():
    ...
    assert isinstance(async_sparklify, quaynor.Tool)
    assert await async_sparklify("foobar") == "✨FOOBAR✨"


def test_async_tool_calling(model):
    chat = quaynor.Chat(model, tools=[async_sparklify])
    chat.ask(
        "Please sparklify this word: 'julemand' and show me the result"
    ).completed()
...
def test_bash_tool(model):
    chat = quaynor.Chat(
        model,
        tools=[quaynor.bash_tool(max_commands=1000)],
        template_variables={"enable_thinking": False},
    )
