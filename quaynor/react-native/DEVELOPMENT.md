# quaynor — React Native bindings (UniFFI)
...
├── Quaynor.podspec            # CocoaPods spec (iOS) — customized, do not regenerate
│
...
│   └── NativeQuaynor.ts      # TurboModule spec (generated, do not edit)
│
...
│   ├── ts/
│   │   ├── quaynor.ts         # TypeScript bindings
│   │   └── quaynor-ffi.ts     # Low-level FFI types
│   └── cpp/
│       ├── quaynor.cpp        # C++ JSI bridge
│       └── quaynor.hpp        # C++ header
│
├── cpp/                         # TurboModule C++ glue (generated once, rarely changes)
│   ├── react-native-quaynor.cpp
│   ├── react-native-quaynor.h
│   ├── quaynor-react-native.cpp
│   └── quaynor-react-native.h
│
...
npx --prefix react-native uniffi-bindgen-react-native generate jsi bindings \
  ...
  target/debug/libquaynor_uniffi.so
```

**Do not regenerate for:** TypeScript wrapper changes, build config changes, version bumps.

### 2. TurboModule glue (`ios/`, `cpp/`, `src/NativeQuaynor.ts`, `src/index.tsx`)
...
```bash
cd react-native
npx uniffi-bindgen-react-native generate jsi turbo-module --config ubrn.config.yaml quaynor
```

**WARNING:** This overwrites `Quaynor.podspec`, `android/build.gradle`, and `android/CMakeLists.txt` with defaults, destroying custom build logic (binary download, xcframework support, etc.). After running, restore these files:

```bash
git checkout -- Quaynor.podspec android/build.gradle android/CMakeLists.txt
```

**Do not regenerate for:** Rust API changes, adding new functions/types — those only affect the bindings layer above.

### 3. TypeScript wrappers (`src/*.ts` except `NativeQuaynor.ts` and `index.tsx`)
...
- `generated/ts/quaynor.ts` — TypeScript classes, enums, free functions
- `generated/ts/quaynor-ffi.ts` — low-level FFI type bridge
- `generated/cpp/quaynor.{cpp,hpp}` — C++ JSI bridge implementation
...
nix develop .#android --command bash -c \
  'cd quaynor && cargo build -p quaynor-uniffi --target aarch64-linux-android --release'

# Android x86_64 (emulator)
nix develop .#android --command bash -c \
  'cd quaynor && cargo build -p quaynor-uniffi --target x86_64-linux-android --release'
```

Then copy the `.so` files to where the Android build expects them:

```bash
mkdir -p quaynor/react-native/android/build/quaynor-native/{arm64-v8a,x86_64}

# ARM64
cp quaynor/target/aarch64-linux-android/release/libquaynor_uniffi.so \
  quaynor/react-native/android/build/quaynor-native/arm64-v8a/

# x86_64
cp quaynor/target/x86_64-linux-android/release/libquaynor_uniffi.so \
  quaynor/react-native/android/build/quaynor-native/x86_64/
```
...
- **iOS:** `Quaynor.podspec` downloads and extracts `QuaynorFramework.xcframework.zip` from the same release
...
# From project root
nix develop .#android --command bash -c \
  'cd quaynor/react-native/test-app/android && \
...
cd quaynor/react-native/test-app
npx react-native start --port 8081

# Terminal 2: install, set up port forwarding, and launch
adb install -r quaynor/react-native/test-app/android/app/build/outputs/apk/debug/app-debug.apk
adb reverse tcp:8081 tcp:8081
adb shell am start -n com.quaynortest/.MainActivity
```
...
- **`Quaynor.podspec`** — Downloads prebuilt xcframework from GitHub Releases, custom authors/source fields
...
- **`android/src/main/java/ooo/quaynor/QuaynorModule.kt`** — Loads `libquaynor_uniffi.so` before the bridge lib
- **`android/src/main/java/ooo/quaynor/QuaynorPackage.kt`** — Uses `BaseReactPackage` instead of deprecated `TurboReactPackage`
- **`android/cpp-adapter.cpp`** — JNI symbols use `ooo_quaynor` package path (generator defaults to `com_quaynor`)
