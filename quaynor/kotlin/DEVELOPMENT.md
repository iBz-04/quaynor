# quaynor — Kotlin/Android bindings (UniFFI)
...
./kotlin/Scripts/generate-bindings.sh
```

The script builds `quaynor-uniffi`, runs `uniffi-bindgen`, and post-processes the
generated `Message` sealed class (its variant named `Message` needs a fully
qualified supertype reference to compile in Kotlin).
...
- `QUAYNOR_LIB_DIR` — directory containing `libquaynor_uniffi.so` (sets `jna.library.path`)
...
### Public API package: `ai.quaynor`

All consumer-facing types live in `ai.quaynor`. The generated UniFFI bindings are in `uniffi.quaynor` and should not be imported by consumers directly.
...
- The `Message` sealed class in `Exports.kt` (wraps `uniffi.quaynor.Message`)

### Why Message is wrapped

The generated `uniffi.quaynor.Message` sealed class carries an explicit `Role` field on every variant (including a nested variant also named `Message`), which is awkward to consume directly. The hand-written `Message` sealed class in `ai.quaynor` exposes the friendlier `User`/`Assistant`/`System`/`Tool` shape instead, and `Chat.getChatHistory()` / `Chat.setChatHistory()` convert at the boundary.
