...
class IntegrationTest {
    ...
    fun testDownloadModel() = runBlocking {
        val modelUrl = "hf://bartowski/Qwen_Qwen3-0.6B-GGUF/Qwen_Qwen3-0.6B-Q4_K_M.gguf"
    ...
    fun testEncoder() = runBlocking {
        ...
        val a = encoder.encode(text = "Copenhagen is in Denmark.")
        val b = encoder.encode(text = "Berlin is in Germany.")

        assertTrue(a.isNotEmpty())
        assertEquals(a.size, b.size)
        val similarity = cosineSimilarity(a, b)
        assertTrue("similarity should be finite", abs(similarity) <= 1.0f + 1e-5f)
    }
