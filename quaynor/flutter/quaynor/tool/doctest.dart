...
String normalizeImport(String importLine) {
  // Normalize quaynor_dart to quaynor
  importLine = importLine.replaceAll('quaynor_dart/quaynor_dart.dart', 'quaynor/quaynor.dart');
  importLine = importLine.replaceAll("'package:quaynor_dart/", "'package:quaynor/");
  importLine = importLine.replaceAll('"package:quaynor_dart/', '"package:quaynor/');
...
Set<String> extractImports(String code) {
  ...
  for (final match in importRegex.allMatches(code)) {
    ...
    // Skip quaynor imports since we add them ourselves
    if (importLine.contains('quaynor.dart')) {
      continue;
    }
...
/// Remove Quaynor.init() calls — init is handled in setUpAll
String removeInitCalls(String code) {
  final initRegex = RegExp(r'^\s*await\s+quaynor\.Quaynor\.init\(\)\s*;\s*\n?', multiLine: true);
...
String generateTestFile(List<CodeGroup> groups, String testName) {
  ...
  allImports.add("import 'package:quaynor/quaynor.dart' as quaynor;");
  ...
  buffer.writeln("      await quaynor.Quaynor.init();");
  ...
  buffer.writeln("        final hfCache = Directory('\$cacheHome/quaynor/models/bartowski/Qwen_Qwen3-0.6B-GGUF');");
  ...
  buffer.writeln("      final cacheHome = Platform.environment['XDG_CACHE_HOME'] ?? '\${Platform.environment['HOME']}/.cache';");
  buffer.writeln("      final hfLink = Link('\$cacheHome/quaynor/models/bartowski/Qwen_Qwen3-0.6B-GGUF/Qwen_Qwen3-0.6B-Q4_K_M.gguf');");
  buffer.writeln("      if (hfLink.existsSync()) {");
...
void main(List<String> args) async {
  ...
  final packageDir = scriptDir.parent; // flutter/quaynor
