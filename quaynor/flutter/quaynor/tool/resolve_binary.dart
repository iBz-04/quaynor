#!/usr/bin/env dart
// Binary resolution script for Quaynor Flutter plugin
...
String? checkEnvironmentOverride(Config config) {
  if (config.isApplePlatform) {
    ...
    final xcframeworkPath = Platform.environment['QUAYNOR_FLUTTER_XCFRAMEWORK_PATH'];
    if (xcframeworkPath != null && xcframeworkPath.isNotEmpty) {
      ...
      if (xcframeworkDir.existsSync()) {
        ...
        throw Exception(
          'QUAYNOR_FLUTTER_XCFRAMEWORK_PATH is set but path does not exist: $xcframeworkPath'
        );
      }
    }
  } else {
    // For desktop/Android, check for library path
    final libPath = Platform.environment['QUAYNOR_FLUTTER_LIB_PATH'];
    if (libPath != null && libPath.isNotEmpty) {
      ...
      if (libFile.existsSync()) {
        ...
        throw Exception(
          'QUAYNOR_FLUTTER_LIB_PATH is set but file does not exist: $libPath'
        );
      }
    }
  }
...
String? checkLocalBuild(Config config) {
  ...
  final pluginDir = toolDir.parent; // quaynor/
  final flutterDir = pluginDir.parent; // flutter/
  final quaynorDir = flutterDir.parent; // quaynor/
  final targetDir = Directory('${quaynorDir.path}/target');
  ...
  if (config.isApplePlatform) {
    ...
    for (final triple in triples) {
      final dylibFile = File('${targetDir.path}/$triple/${config.buildType}/libquaynor_flutter.dylib');
      if (dylibFile.existsSync()) {
        throw Exception(
          ...
          'Or set QUAYNOR_FLUTTER_XCFRAMEWORK_PATH to point to your xcframework.'
        );
      }
    }
    return null;
...
String? checkCachedDownload(Config config) {
  ...
  final cacheBasePath = '${config.cacheDir}/quaynor/$version';

  if (config.isApplePlatform) {
    // Check for cached xcframework
    final xcframeworkPath = '$cacheBasePath/xcframework/quaynor_flutter.xcframework';
    final xcframeworkDir = Directory(xcframeworkPath);
    if (xcframeworkDir.existsSync()) {
...
Future<String> downloadFromGitHub(Config config) async {
  ...
  if (config.isApplePlatform && config.buildType == 'debug') {
    throw Exception(
      ...
      'For local development, set QUAYNOR_FLUTTER_XCFRAMEWORK_PATH to point to your xcframework.\n'
...
Future<String> downloadXCFramework(Config config, String version) async {
  ...
  final xcframeworkPath = '$cacheDir/quaynor_flutter.xcframework';
