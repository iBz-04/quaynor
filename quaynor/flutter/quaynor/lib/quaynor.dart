...
export 'src/rust/frb_generated.dart' show Quaynor;

import 'src/rust/lib.dart' as quaynor;
...
List<quaynor.PromptPart> _convertPromptParts(List<PromptPart> parts) {
  return parts.map((p) => switch (p) {
    TextPart(:final text) => quaynor.PromptPart.text(content: text),
    ImagePart(:final path) => quaynor.PromptPart.image(path: path),
    AudioPart(:final path) => quaynor.PromptPart.audio(path: path),
  }).toList();
}
...
extension ToolCallExtension on quaynor.ToolCall {
  /// Get the arguments as a JSON string
  String get argumentsJson => quaynor.toolCallArgumentsJson(toolCall: this);

  /// Get the arguments as a parsed Map
  Map<String, dynamic> get argumentsMap =>
      json.decode(quaynor.toolCallArgumentsJson(toolCall: this))
...
class Tool {
  ...
  factory Tool({
    ...
    final tool = quaynor.newToolImpl(
  ...
  factory Tool.python({
    ...
    final rustTool = quaynor.newPythonTool(
  ...
  factory Tool.bash({
    ...
    final rustTool = quaynor.newBashTool(
  ...
  quaynor.RustTool get _internalTool => _tool;
}
...
class TokenStream extends Stream<String> {
  final quaynor.RustTokenStream _tokenStream;
...
class Chat {
  ...
  /// ```
  factory Chat({
    required quaynor.Model model,
    String? systemPrompt,
    int contextSize = 4096,
    bool? allowThinking = null,
    Map<String,bool> templateVariables = const {},
    List<Tool> tools = const [],
    quaynor.SamplerConfig? sampler,
  }) {
    final chat = quaynor.RustChat(
      model: model,
  ...
  static Future<Chat> fromPath({
    ...
    final chat = await quaynor.RustChat.fromPath(
      modelPath: modelPath,
  ...
  Future<List<quaynor.Message>> getChatHistory() => _chat.getChatHistory();

  /// Set the chat history.
  Future<void> setChatHistory(List<quaynor.Message> messages) =>
  ...
  Future<void> setSamplerConfig(quaynor.SamplerConfig samplerConfig) =>
      _chat.setSamplerConfig(samplerConfig: samplerConfig);

  /// Get the current sampler configuration.
  Future<quaynor.SamplerConfig> getSamplerConfig() =>
