...
void main() {
  group('A group of tests', () {
    ...
    setUp(() async {
      ...
      final multiplyStringsTool = quaynor.Tool(
      ...
      final setIntersectionTool = quaynor.Tool(
        function: setIntersection,
        name: "setIntersection",
        description: "Returns the intersection between set1 and set2.",
      );

      final sparklify_tool = quaynor.Tool(
        function: sparklify,
        name: "sparklify",
      ...
      final multiplySquareMatricesTool = quaynor.Tool(
      ...
      chat = await quaynor.Chat.fromPath(
        modelPath: modelPath,
        systemPrompt: "",
        contextSize: 2048,
    ...
    test('Tool calling with no arguments test', () async {
      ...
      final toolCall = history.firstWhere((m) => m.role == quaynor.Role.tool);
    ...
    test('Tool call message has correct structure in chat history', () async {
      ...
      final toolCallsMessage =
          messages.firstWhere(
                (m) => m is quaynor.Message_ToolCalls,
                orElse: () =>
                    throw Exception('No tool calls message found in history'),
              )
              as quaynor.Message_ToolCalls;

      // Check the role is assistant
      expect(toolCallsMessage.role, equals(quaynor.Role.assistant));
      ...
      final toolRespMessage =
          messages.firstWhere(
                (m) => m is quaynor.Message_ToolResp,
                orElse: () => throw Exception(
                  'No tool response message found in history',
                ),
              )
              as quaynor.Message_ToolResp;

      // Check the tool response
      expect(toolRespMessage.role, equals(quaynor.Role.tool));
    ...
    test('Tools work with custom sampler', () async {
      final sampler = quaynor.SamplerBuilder()
    ...
    test('Tools work with sampler presets', () async {
      final sampler = quaynor.SamplerPresets.temperature(temperature: 1.2);
      await chat!.setSamplerConfig(sampler);
      final response = await chat!
          .ask("Can you please strongify the string 'Wrawr'?")
          .completed();
    ...
    test('Sampler actually affects output', () async {
      ...
      final greedy = quaynor.SamplerPresets.greedy();
    ...
    test('Cosine similarity works', () {
      ...
      final similarity = quaynor.cosineSimilarity(a: vec1, b: vec2);
      ...
      final selfSim = quaynor.cosineSimilarity(a: vec1, b: vec1);
      ...
      final orthogonalSim = quaynor.cosineSimilarity(
        a: orthogonal1,
        b: orthogonal2,
      );
      ...
      final oppositeSim = quaynor.cosineSimilarity(
        a: opposite1,
        b: opposite2,
      );
    ...
    test('set_tools changes available tools', () async {
      ...
      final testChat = await quaynor.Chat.fromPath(
        modelPath: modelPath,
        systemPrompt: "",
        contextSize: 1024,
        templateVariables: {"enable_thinking" : false},
    ...
    test('Python tool hello world', () async {
      final pythonChat = await quaynor.Chat.fromPath(
        modelPath: modelPath,
        allowThinking: false,
        tools: [quaynor.Tool.python()],
      );
      ...
      final toolCalls = history
          .whereType<quaynor.Message_ToolCalls>()
          .expand((m) => m.toolCalls)
          .toList();
      final toolResponses = history
          .whereType<quaynor.Message_ToolResp>()
          .toList();
    ...
    test('Encoder.fromPath creates encoder and encodes text', () async {
      ...
      final encoder = await quaynor.Encoder.fromPath(modelPath: modelPath);
      ...
      final selfSim = quaynor.cosineSimilarity(a: embeddings, b: embeddings);
      expect(selfSim, closeTo(1.0, 0.001));
    });
    ...
    test(
      ...
      () async {
        ...
        final crossEncoder = await quaynor.CrossEncoder.fromPath(
          modelPath: modelPath,
        );
        final documents = [
  ...
  group('Message struct tests', () {
    test('Message.message constructor creates correct instance', () {
      final msg = quaynor.Message.message(
        role: quaynor.Role.user,
        content: 'Hello, world!',
      );

      expect(msg, isA<quaynor.Message_Message>());
      expect(
        (msg as quaynor.Message_Message).role,
        equals(quaynor.Role.user),
      );
      expect(msg.content, equals('Hello, world!'));
    });

    test('Message.message with different roles', () {
      final userMsg = quaynor.Message.message(
        role: quaynor.Role.user,
        content: 'User message',
      );
      final assistantMsg = quaynor.Message.message(
        role: quaynor.Role.assistant,
        content: 'Assistant message',
      );
      final systemMsg = quaynor.Message.message(
        role: quaynor.Role.system,
        content: 'System message',
      );

      expect(
        (userMsg as quaynor.Message_Message).role,
        equals(quaynor.Role.user),
      );
      expect(
        (assistantMsg as quaynor.Message_Message).role,
        equals(quaynor.Role.assistant),
      );
      expect(
        (systemMsg as quaynor.Message_Message).role,
        equals(quaynor.Role.system),
      );
    });
    ...
    test('Message.message with empty content', () {
      ...
      expect((msg as quaynor.Message_Message).content, equals(''));
    });
    ...
    test('Message.message with multiline content', () {
      ...
      expect((msg as quaynor.Message_Message).content, equals(content));
    });
    ...
    test('Message.toolResp constructor creates correct instance', () {
      final msg = quaynor.Message.toolResp(
        role: quaynor.Role.tool,
        name: 'calculator',
        content: '42',
      );

      expect(msg, isA<quaynor.Message_ToolResp>());
      expect(
        (msg as quaynor.Message_ToolResp).role,
        equals(quaynor.Role.tool),
      );
    ...
    test('Message.toolResp with JSON content', () {
      ...
      expect((msg as quaynor.Message_ToolResp).content, equals(jsonContent));
    });

    test('Role enum has all expected values', () {
      expect(quaynor.Role.values, contains(quaynor.Role.user));
      expect(quaynor.Role.values, contains(quaynor.Role.assistant));
      expect(quaynor.Role.values, contains(quaynor.Role.system));
      expect(quaynor.Role.values, contains(quaynor.Role.tool));
      expect(quaynor.Role.values.length, equals(4));
    });

    test('Message variants are distinguishable', () {
      final textMsg = quaynor.Message.message(
        role: quaynor.Role.user,
        content: 'Hello',
      );
      final toolRespMsg = quaynor.Message.toolResp(
        role: quaynor.Role.tool,
        name: 'test_tool',
        content: 'result',
      );

      expect(textMsg, isA<quaynor.Message_Message>());
      expect(textMsg, isNot(isA<quaynor.Message_ToolResp>()));
      expect(toolRespMsg, isA<quaynor.Message_ToolResp>());
      expect(toolRespMsg, isNot(isA<quaynor.Message_Message>()));
    });

    test('Message equality works correctly', () {
      final msg1 = quaynor.Message.message(
        role: quaynor.Role.user,
        content: 'Hello',
      );
      final msg2 = quaynor.Message.message(
        role: quaynor.Role.user,
        content: 'Hello',
      );
      final msg3 = quaynor.Message.message(
        role: quaynor.Role.user,
        content: 'Different',
      );
    ...
    test('Message can be used in collections', () {
      final messages = <quaynor.Message>[
        quaynor.Message.message(
          role: quaynor.Role.system,
          content: 'You are helpful',
        ),
        quaynor.Message.message(role: quaynor.Role.user, content: 'Hi'),
        quaynor.Message.message(
          role: quaynor.Role.assistant,
          content: 'Hello!',
        ),
      ...
      expect(messages[0], isA<quaynor.Message_Message>());
      expect(
        (messages[1] as quaynor.Message_Message).role,
        equals(quaynor.Role.user),
      );
    });
    ...
    test('Message copyWith works for Message_Message', () {
      final original =
          quaynor.Message.message(
              ...
              as quaynor.Message_Message;

      final modified = original.copyWith(content: 'Modified');

      expect(modified.role, equals(quaynor.Role.user));
    ...
    test('Message copyWith works for Message_ToolResp', () {
      ...
      expect(modified.role, equals(quaynor.Role.tool));
