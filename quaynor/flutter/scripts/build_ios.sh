...
# Build Quaynor for iOS development, heavily vibe-coded.
...
QUAYNOR_WORKSPACE_DIR="$(cd "$FLUTTER_DIR/.." && pwd)"
TARGET_DIR="$QUAYNOR_WORKSPACE_DIR/target"
XCFRAMEWORK_OUTPUT="$TARGET_DIR/xcframework/quaynor_flutter.xcframework"
...
if [ "$SKIP_BUILD" = false ]; then
    ...
    cargo build -p quaynor-flutter --target aarch64-apple-ios $CARGO_PROFILE_FLAG --manifest-path "$QUAYNOR_WORKSPACE_DIR/Cargo.toml"

    echo ""
    echo "Step 2/4: Building for iOS simulator (aarch64-apple-ios-sim)..."
    cargo build -p quaynor-flutter --target aarch64-apple-ios-sim $CARGO_PROFILE_FLAG --manifest-path "$QUAYNOR_WORKSPACE_DIR/Cargo.toml"

    echo ""
    echo "Step 3/4: Building for iOS simulator (x86_64-apple-ios)..."
    cargo build -p quaynor-flutter --target x86_64-apple-ios $CARGO_PROFILE_FLAG --manifest-path "$QUAYNOR_WORKSPACE_DIR/Cargo.toml"
else
    echo ""
...
lipo -create \
    "$TARGET_DIR/aarch64-apple-ios-sim/$BUILD_TYPE/libquaynor_flutter.dylib" \
    "$TARGET_DIR/x86_64-apple-ios/$BUILD_TYPE/libquaynor_flutter.dylib" \
    -output "$TARGET_DIR/universal-ios-sim/$BUILD_TYPE/libquaynor_flutter.dylib"

# Set install name for iOS simulator
install_name_tool -id @rpath/quaynor_flutter.framework/quaynor_flutter \
    "$TARGET_DIR/universal-ios-sim/$BUILD_TYPE/libquaynor_flutter.dylib"

# Create iOS simulator framework structure
IOS_SIM_FRAMEWORK="$TARGET_DIR/universal-ios-sim/$BUILD_TYPE/quaynor_flutter.framework"
mkdir -p "$IOS_SIM_FRAMEWORK"
cp "$TARGET_DIR/universal-ios-sim/$BUILD_TYPE/libquaynor_flutter.dylib" \
    "$IOS_SIM_FRAMEWORK/quaynor_flutter"
...
<dict>
    ...
    <string>quaynor_flutter</string>
    <key>CFBundleIdentifier</key>
    <string>ooo.quaynor.flutter</string>
...
install_name_tool -id @rpath/quaynor_flutter.framework/quaynor_flutter \
    "$TARGET_DIR/aarch64-apple-ios/$BUILD_TYPE/libquaynor_flutter.dylib"

# Create iOS device framework structure
IOS_DEVICE_FRAMEWORK="$TARGET_DIR/aarch64-apple-ios/$BUILD_TYPE/quaynor_flutter.framework"
mkdir -p "$IOS_DEVICE_FRAMEWORK"
cp "$TARGET_DIR/aarch64-apple-ios/$BUILD_TYPE/libquaynor_flutter.dylib" \
    "$IOS_DEVICE_FRAMEWORK/quaynor_flutter"
...
echo ""
echo "  export QUAYNOR_FLUTTER_XCFRAMEWORK=\"$XCFRAMEWORK_OUTPUT\""
echo "  cd $FLUTTER_DIR/example_app"
echo "  flutter run"
echo ""
echo "Or run this one-liner:"
echo ""
echo "  QUAYNOR_FLUTTER_XCFRAMEWORK=\"$XCFRAMEWORK_OUTPUT\" flutter run"
echo "========================================"
